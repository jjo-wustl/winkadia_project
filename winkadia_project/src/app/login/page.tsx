...
export default function LoginPage() { // 로그인, 회원가입, 비밀번호 재설정 화면을 담당하는 페이지 컴포넌트임
  ...
  const pageTitle =
    mode === "login"
      ? "당신을 윙카디아의 성역으로 초대합니다"
      : mode === "signup"
        ? "윙카디아의 새로운 인연을 기록합니다"
        : "잃어버린 인장을 다시 찾아드립니다"; // 현재 모드에 맞는 로그인 카드 제목을 정함
    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-[#fbf7ff] via-[#f3e8ff] to-[#ead8ff] text-[#3a3047]"> {/* 로그인 페이지 전체를 감싸는 최상위 영역이며 연보라 그라데이션 배경을 적용함 */}
      ...
      <div className="relative z-20 flex min-h-screen flex-col px-5 py-6"> {/* 실제 로그인 콘텐츠를 배경 위에 올리는 영역임 */}
        ...
        <main className="flex flex-1 items-center justify-center py-8"> {/* 로그인 카드와 장식 이미지를 화면 중앙 영역에 배치함 */}
          <div className="relative flex w-full max-w-[940px] justify-center"> {/* 로그인 카드, 왼쪽 장식, 여자고양이 이미지의 위치 기준이 되는 래퍼임 */}
            <section className="relative z-30 flex min-h-[840px] w-full max-w-[590px] flex-col items-center justify-center rounded-[34px] border border-[#d8c7f6]/80 bg-white/80 px-7 py-12 text-center shadow-[0_28px_90px_rgba(118,82,166,0.22)] backdrop-blur-xl sm:min-h-[880px] sm:rounded-[42px] sm:px-12 sm:py-14"> {/* 글씨가 커진 만큼 로그인 창 폭과 높이를 살짝 늘림 */}
              <img
              ...
              <div className="relative z-30 mx-auto mb-12 flex w-full max-w-[450px] flex-col items-center justify-center pt-14 text-center sm:pt-16"> {/* 로그인 카드 상단 문구 영역을 가운데 정렬하고 장식 이미지와 겹치지 않게 위쪽 여백을 둠 */}
                <img
                  src="/logo.png"
                  alt="WINKADIA"
                ...
                <div className="mx-auto mb-8 flex w-fit items-center justify-center gap-2 rounded-full bg-[#eee5ff]/95 px-5 py-2 text-center text-[18px] font-semibold tracking-wide text-[#8b68d8] shadow-sm">
                  <span className="flex h-5 w-5 items-center justify-center text-center">⌂</span>
              ...
              <div className="relative z-30 mx-auto flex w-full max-w-[450px] flex-col items-center justify-center gap-6 text-center"> {/* 폼 전체 폭을 넓혀 큰 글씨가 답답하지 않게 함 */}
                {mode !== "reset" && (
                  <button
                    type="button"
                    onClick={handleGoogle}
                    disabled={loading}
                    className="relative flex h-[76px] w-full items-center justify-center rounded-[26px] border border-[#d9c8f4] bg-white/60 px-[84px] text-center text-[25px] font-semibold text-[#7c68a0] shadow-sm transition hover:-translate-y-0.5 hover:bg-white/84 disabled:cursor-not-allowed disabled:opacity-60"
                  > {/* 구글 로그인 버튼 글씨와 높이를 더 키움 */}
                    <span className="absolute left-5 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-white text-xl font-bold shadow-sm">
                      G
                    </span>
                    <span className="pointer-events-none absolute left-[64px] top-1/2 h-9 w-px -translate-y-1/2 bg-[#dacdf1]" /> {/* 커진 아이콘 영역에 맞춰 구분선 위치와 높이를 조정함 */}
                    <span className="block w-full text-center">{t.auth.loginWithGoogle}</span>
                  </button>
                )}

                <form onSubmit={handleEmail} className="mx-auto flex w-full max-w-[450px] flex-col items-center justify-center gap-6 text-center"> {/* 이메일 로그인, 회원가입, 비밀번호 재설정에 사용하는 form이며 인풋 사이 간격을 일정하게 맞춤 */}
                  {mode === "signup" && (
                    <div className="relative mx-auto w-full">
                      <UserRound className="pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 text-[#8e71d1]" size={24} />
                      <span className="pointer-events-none absolute left-[64px] top-1/2 h-9 w-px -translate-y-1/2 bg-[#dacdf1]" /> {/* 인풋 안에서 아이콘 영역과 텍스트 영역을 나누는 선임 */}
                      <input
                        type="text"
                        value={displayName}
                        onChange={(e) => setDisplayName(e.target.value)}
                        className="h-[76px] w-full rounded-[26px] border border-[#d9c8f4] bg-white/60 px-[88px] text-center text-[26px] font-semibold text-[#4b4058] outline-none transition placeholder:text-center placeholder:text-[#a89ab8] focus:border-[#9d78db] focus:bg-white/84 focus:shadow-[0_0_0_4px_rgba(157,120,219,0.12)]"
                        required
                    ...
                    <div className="grid w-full grid-cols-3 gap-3"> {/* 회원가입 모드에서 성별 선택 버튼을 3칸으로 보여줌 */}
                      {(
                        ...
                        <button
                          ...
                          className="rounded-[18px] border px-2 py-3 text-center text-[19px] font-semibold transition"
                          style={{
                            borderColor:
                  ...
                  <div className="relative mx-auto w-full">
                    <Heart className="pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 text-[#8e71d1]" size={24} />
                    <span className="pointer-events-none absolute left-[64px] top-1/2 h-9 w-px -translate-y-1/2 bg-[#dacdf1]" /> {/* 인풋 안에서 아이콘 영역과 텍스트 영역을 나누는 선임 */}
                    <input
                      type="email"
                      value={email}
                      ...
                      className="h-[76px] w-full rounded-[26px] border border-[#d9c8f4] bg-white/60 px-[88px] text-center text-[26px] font-semibold text-[#4b4058] outline-none transition placeholder:text-center placeholder:text-[#a89ab8] focus:border-[#9d78db] focus:bg-white/84 focus:shadow-[0_0_0_4px_rgba(157,120,219,0.12)]"
                      required
                      placeholder={mode === "reset" ? t.auth.email : "이메일"}
                    />
                  </div>

                  {mode !== "reset" && (
                    <div className="relative mx-auto w-full">
                      <Lock className="pointer-events-none absolute left-5 top-1/2 -translate-y-1/2 text-[#8e71d1]" size={24} />
                      <span className="pointer-events-none absolute left-[64px] top-1/2 h-9 w-px -translate-y-1/2 bg-[#dacdf1]" /> {/* 인풋 안에서 아이콘 영역과 텍스트 영역을 나누는 선임 */}
                      <input
                  ...
                  {error && (
                    <div className="mx-auto w-full rounded-[18px] border border-pink-200 bg-pink-50/80 px-4 py-3 text-center text-[18px] font-medium text-pink-500"> {/* 에러 메시지가 있을 때만 에러 박스를 보여줌 */}
                      ⚠ {error}
                    </div>
                  )}

                  {resetSent && (
                    <div className="mx-auto w-full rounded-[18px] border border-purple-200 bg-purple-50/80 px-4 py-3 text-center text-[18px] font-medium text-[#7f61c8]"> {/* 비밀번호 재설정 이메일 발송에 성공했을 때 안내 박스를 보여줌 */}
                      ✦ {t.auth.resetSent}
                    </div>
                  )}
                  ...
                  <button
                    type="submit"
                    disabled={loading}
                    className="relative mx-auto mt-2 flex h-[76px] w-full items-center justify-center rounded-[26px] bg-gradient-to-r from-[#8d67d6] via-[#8157ca] to-[#7350be] px-6 text-center text-[26px] font-bold text-white shadow-[0_16px_36px_rgba(125,86,200,0.34)] transition hover:-translate-y-0.5 hover:shadow-[0_20px_42px_rgba(125,86,200,0.42)] disabled:cursor-not-allowed disabled:opacity-60"
                  > {/* 현재 모드에 맞는 제출 버튼이며 버튼 문구를 더 크게 보여줌 */}
                    <Sparkles size={22} className="absolute left-6 top-1/2 -translate-y-1/2" />
                ...
                <div className="flex w-full flex-col items-center justify-center text-center"> {/* 로그인, 회원가입, 비밀번호 재설정 모드를 전환하는 하단 영역임 */}
                  {mode === "login" && (
                    <div className="flex w-full flex-col items-center justify-center space-y-5 text-center">
                      <label className="inline-flex items-center justify-center gap-2 text-center text-[21px] font-semibold text-[#9586a7]"> {/* 참고 이미지처럼 로그인 상태 유지 표시를 보여줌 */}
                        <input
                          type="checkbox"
                          defaultChecked
                          className="h-5 w-5 accent-[#8d67d6]"
                        />
                        <span>로그인 상태 유지</span>
                      </label>

                      <div className="flex items-center justify-center gap-4 text-center text-[21px] font-semibold text-[#9586a7]">
                        <button
                          type="button"
                          onClick={() => {
                            setMode("signup");
                  ...
                  {mode === "signup" && (
                    <p className="text-center text-[21px] text-[#9282a4]">
                      {t.auth.hasAccount}{" "}
                      <button
                        type="button"
                        onClick={() => {
                  ...
                  {mode === "reset" && (
                    <button
                      ...
                      className="text-center text-[21px] font-bold text-[#805dd0] transition hover:text-[#6543b8]"
                    >
                      ← {t.common.back}
                    </button>
                  )}
        ...
        <footer className="pb-5 text-center text-base font-semibold tracking-wide text-[#9b86bd]"> {/* 로그인 화면 하단 저작권 문구 영역임 */}
