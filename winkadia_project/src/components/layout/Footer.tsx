...
const socialLinks = [
  ...
  },
  {
    label: "Mail",
    href: "mailto:",
    icon: "/images/footer/mail.png",
    external: false,
  },
]; // 푸터에 보여줄 SNS 아이콘 이미지와 이동 경로를 모아둠
          ...
          <div className="relative z-10"> {/* 실제 푸터 콘텐츠를 그라데이션 위에 올림 */}
            ...
            <div className="mb-3 flex items-center justify-center gap-2.5 sm:mb-4 sm:gap-4 lg:mb-5"> {/* SNS 아이콘 이미지 버튼들을 가로로 배치함 */}
              {socialLinks.map((item) => (
                ...
                > {/* 각 SNS 또는 연락 링크로 이동하는 원형 아이콘 버튼임 */}
                  <img
                    src={item.icon}
                    alt=""
            ...
            <div className="hidden items-center justify-center gap-4 text-[13px] font-black text-[#5f4b8f] sm:flex sm:text-[15px] lg:text-[18px]"> {/* 푸터 약관, 개인정보, 문의 문구를 표시함 */}
              <button type="button" className="transition hover:text-[#6f4fbd]">
                {t.footer.terms}
              </button>
